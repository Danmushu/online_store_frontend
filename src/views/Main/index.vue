      ...
      <main>
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
          <div class="py-24 text-center">
            <h1 class="text-4xl font-bold tracking-tight text-gray-900">欢迎来到商城</h1>
            <p class="mx-auto mt-4 max-w-3xl text-base text-gray-500">为工作场所、家庭和旅行精心设计的对象</p>
          </div>
...
<!--          </section>-->
          ...
          <section aria-labelledby="products-heading" class="mt-8">
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:gap-x-8">
              <a v-for="product in products" @click="$router.push({path:'/detail', query:{id:product.id}})"
                 :key="product.name" :href="product.href" class="group">
...
const products = ref([])
onMounted(()=>{ getProduct({limit: 9, offset: 0}).then(res => res.json().then(data=> {
